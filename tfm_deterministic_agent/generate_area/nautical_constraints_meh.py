...
import pandas as pd
from shapely.geometry import Point, Polygon, LineString, MultiLineString, MultiPolygon, shape
from typing import List, Union

def create_buoy_nodes(buoy_geoms: List[Union[Point, Polygon, LineString, dict, list]]) -> pd.DataFrame:
    """
    Genera DataFrame de boyas de peligro como nodos bloqueados,
    aceptando Points, Polygons, LineStrings (usa punto medio),
    dicts GeoJSON o listas anidadas.
    """
    rows = []
    ...
    for b in flat:
        ...
        if isinstance(b, Point):
            ...
            pt = b.interpolate(0.5, normalized=True)
        elif isinstance(b, (Polygon, MultiPolygon, MultiLineString)):
            pt = b.centroid
        elif isinstance(b, dict) and 'geometry' in b:
            geom = shape(b['geometry'])
            # recur en caso de ser LineString o Polygon
            if isinstance(geom, LineString):
                pt = geom.interpolate(0.5, normalized=True)
            else:
                pt = geom.centroid
        else:
            raise ValueError(f"Tipo no soportado en buoy_geoms: {type(b)}")
